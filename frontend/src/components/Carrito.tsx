...
const Carrito = () => {
  ...
  return (
    <div>
      <h1 className="text-xl sm:text-2xl font-bold text-slate-800 mb-6">Tu Carrito de Compras</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            {cart.map((item) => (
              <div
                key={item.id}
                className="flex items-center justify-between gap-3 bg-white p-3 sm:p-4 rounded-lg border border-slate-200 shadow-sm"
              >
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <img src={item.img} alt={item.nombre} className="w-14 h-14 sm:w-16 sm:h-16 shrink-0 object-cover rounded-md" />
                ...
                <div className="flex items-center gap-3 sm:gap-6 shrink-0">
                  <p className="font-bold text-indigo-600">${(item.precio * item.cantidad).toFixed(2)}</p>
                  <button
