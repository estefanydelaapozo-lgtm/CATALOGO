          ...
          <div className="bg-white p-6 rounded-lg border border-slate-200 shadow-sm">
            ...
            <div className="space-y-3">
              {METODOS_PAGO.map((metodo) => (
                <label key={metodo} className="flex items-center gap-3 p-3 rounded-lg border border-slate-200 cursor-pointer hover:border-indigo-300 transition">
                  <input
                    type="radio"
                    name="metodoPago"
                    value={metodo}
          ...
          <div className="space-y-3 border-b border-slate-100 pb-4 mb-4">
            {cart.map((item) => (
              <div key={item.id} className="flex justify-between text-sm">
                ...
                <span className="font-semibold text-slate-800">${(item.precio * item.cantidad).toFixed(2)}</span>
              </div>
            ))}
          </div>
